...
- Auth endpoints (`internal/api`): `GET /whoami` (public), `/keys` CRUD, `GET/PUT /anonymous-access`, `GET /admin/audit-log` (admin), `POST /tickets` (key with listen). SQL in `internal/database/api_keys.go` plus the `auth_settings` (anonymous policy, ticket secret, retired public token, `forgotten_public_tokens` kept after `access forget-retired-token` so `keys import` still refuses them, `weak_legacy_keys`) and `audit_log` tables. `data_fixups` rows of the auth upgrade: `import-legacy-auth`, `bootstrap-admin-key`, `removed-user-accounts`, `removed-user-accounts-logged`, and `schema-created-with-api-key-auth` (inserted by `schema.sql` itself: the schema was created by this version, so the legacy import has nothing to import).
...
- `internal/ingest/handler_upload.go` — `ProcessUploadedCall` (full pipeline: identity resolution, dedup, call creation, audio save, SSE publish, transcription enqueue), `ProcessUpload` adapter (implements `api.CallUploader`; `validateUploadMeta` refuses with 400 `invalid_body` a short name containing `/`, `\`, `..` or control characters or longer than 128 bytes, a talkgroup ≤ 0, a missing or non-positive start time, and a start time more than 10 minutes in the future), `ParseRdioScannerFields`, `ParseOpenMHzFields`. `ProcessUploadedCall` also answers 400 `invalid_body` when the call insert finds no partition and `ensurePartitionsFor` refuses the month (`errPartitionOutOfRange`: outside 12 months back to 3 ahead). Uploaded audio is stored at `upload/<system_id>/<YYYY-MM-DD>/<call_id>.<ext>` (`ext` m4a/mp3/wav/ogg/bin from `audioType` or the file name; every part chosen by the engine) and never overwrites a file: if the name is taken, the existing file is left untouched and the new upload is saved as `<call_id>-<hex>.<ext>` (`newUploadAudioKey`, with a WARN).
- Root pipeline (wired in `buildRouter`, `internal/api/server.go`), in order: RequestID (client value kept only if ≤64 chars of `[A-Za-z0-9._-]`), CORS (`Access-Control-Allow-Origin: *` on every response, never credentials; `OPTIONS` → 204 before auth/routing), GetHead (HEAD served by the GET handler), Logger (zerolog/hlog access line for **every** request, refusals and recovered panics included; WebSocket upgrades are logged as 101 when the socket closes), Recoverer (inside Logger: JSON 500, and the panic logged as "recovered from panic" with its stack, method and path through the request's logger; `ResponseTimeout` wraps handlers in `keepPanicStack` so the original stack survives `http.TimeoutHandler`; `http.ErrAbortHandler` is re-raised), APIHeaders (`Cache-Control: no-store` + `Vary: Authorization` on `/api/v1`), Match (`root.Find` with a fresh route context; unmatched → chi 404/405), Resolve principal (`Authorization: Bearer` only; `?ticket=` only on ticket routes for GET/HEAD and it wins; invalid credential → 401, never anonymous), Authorize (public / `key_required` / `insufficient_scope` / `restricted_credential`), Audit (key principals the auth layer let through, non-GET/HEAD/OPTIONS). Rate limiting is interleaved with Resolve: per-IP for no credential (including uploads whose key is in a form field), tickets, legacy keys and key-cache misses (client IP from `TrustedProxies.ClientIP` in `clientip.go`, which only believes `X-Forwarded-For`/`X-Real-IP` from `TRUSTED_PROXIES` peers and walks the chain right-to-left); per-key when the key has `rate_limit_rps`. Then route-group middleware: MaxBodySize (10 MB for API, 50 MB for uploads), the upload middleware (key from multipart `key`/`api_key` when no Bearer header), ResponseTimeout (wraps non-SSE/audio handlers with `HTTP_WRITE_TIMEOUT`). The server limits the request line plus headers to 64 KiB (`MaxHeaderBytes`; net/http adds 4 KiB of read slack, so the 431 comes above about 68 KiB; clients should stay under 64 KiB). The pipeline's code is split across:
  - `internal/api/middleware.go` — RequestID, CORS, Logger, Recoverer, APIHeaders, MaxBodySize, ResponseTimeout (with `keepPanicStack`).
  - `internal/api/pipeline.go` — Match, Resolve, Authorize, Audit (path capped at 1024 bytes, `database.TruncateAuditPath`), UploadAuth (the upload middleware).
  - `internal/api/authn.go` — credential and ticket resolution (`resolve`, `resolveKey`, `resolveTicket`), the retired-public-token check, per-IP and per-key limits.
  - `internal/api/keycache.go` — the key cache (separate positive/negative LRUs, 30 s TTL, cleared on an auth-generation bump) and the per-IP/per-key limiter sets.
  - `internal/api/streamauth.go` — `streamGuard`, the re-check of long-lived connections (SSE and `/audio/live`): every 60 s, on every auth-generation bump and at the key's `expires_at`; swaps the principal while `listen` remains, otherwise closes with `event: auth` / WebSocket 4401/4403. A re-check that can't reach the database keeps the stream open until the next one.
...
- `internal/audio/router.go` — Audio router: identity resolution (short_name → system/site; sender IP → instance from `STREAM_SOURCE_MAP` or auto-learned), multi-site deduplication, per-talkgroup encoding, publishes to AudioBus. Attribution is derived again for every chunk (real TR instances preferred over the file-watch identity, preferred over the upload identity; only source-map entries remove instances from other senders' candidates). A sender whose short name the preferred instances use for **different** systems is not guessed: its audio is dropped with a WARN (at most every 5 min) until `STREAM_SOURCE_MAP` maps it, since restrictions on live audio depend on correct system attribution.
- `internal/audio/bus.go` — Pub/sub event bus for audio frames. WebSocket clients subscribe with filters (system IDs, TGIDs).
- `internal/api/audio_stream.go` — WebSocket endpoint (`GET /audio/live`). Clients send JSON subscribe/unsubscribe messages; server sends binary frames (12-byte header + audio data), plus a keepalive message and a WebSocket ping every 15 s (`active_streams` counts only allowed talkgroups for restricted principals). Closes a peer silent for 60 s (pongs included) and a client message over 64 KiB (1009). `AudioStreamer.SubscribeAudio` and `AudioBus.Subscribe` take the principal holder (`*atomic.Pointer[auth.Principal]`) that `streamGuard` swaps.
...
- `web/auth.js` — Shared browser-side auth for all web pages (`auth.js?v=3`). Keeps one API key in `localStorage['tr-engine-api-key']` (old `tr-engine-token`/`-write-token`/`-jwt` entries are removed) and fetches `GET /api/v1/whoami` asynchronously. Patches `window.fetch` for **same-origin `/api/` URLs only** to add `Authorization: Bearer` when a key is stored; prompts for a key only on 401 `invalid_key`, or `key_required` with no key and anonymous access off. Replaces `EventSource` for every same-origin `/api/` URL by a wrapper that handles `event: auth` and reconnects with `last_event_id`; when a key is stored it mints a ticket (`POST /api/v1/tickets`) before each (re)connect and connects with `?ticket=`. `window.trAuth`: `ready()`, `getKey()`, `setKey()`, `clearKey()`, `cleanKey(raw)`, `whoami()`, `hasScope()`, `showKeyPrompt()`, `mediaUrl(url)` (sync, cached ticket), `ticketUrl(url)` (async), plus deprecated shims for old pages. `cleanKey(raw)` returns `{key, error}` (`error` is `''` when the key can be sent) with the same rules as tr-dashboard's `cleanPastedKey`: removes invisible characters and surrounding typographic quotes, removes straight quotes only around a `tre_` key, trims the ends, refuses tabs, line breaks and anything outside printable ASCII (U+0020-U+007E), and refuses inner spaces only in a `tre_` value; an imported legacy key with inner spaces is accepted. `setKey()` cleans its argument and rejects (error code `invalid_key_format`) a value that can't be a key. At load, only a stored value outside U+0020-U+007E (including tabs and line breaks) is forgotten, since the page refuses such a value (a line break or a character above U+00FF makes `fetch()` throw, a Latin-1 character is sent as one byte that never matches the key, and tabs are refused as paste accidents although browsers send them); a stored value with inner spaces is kept. `admin.html`/`storage.html` take an admin key per tab (`sessionStorage`) and clean the pasted key the same way.
...
- **Monthly partitioning** on high-volume tables: `calls`, `call_frequencies`, `call_transmissions`, `unit_events`, `trunking_messages`. Weekly for `mqtt_raw_messages`. Ingest (MQTT, watch and upload alike) creates a missing monthly partition on demand (`ensurePartitionsFor`) only for months from 12 back to 3 ahead of now; a call outside that window is dropped with a WARN (an upload gets 400 `invalid_body`). The `WATCH_DIR` startup backfill still creates older partitions itself.
...
**Auto-apply on startup:** `schema.sql` is embedded in the binary. On connect, `db.ApplySchemaIfEmpty()` (`InitSchema` is the same without the "applied" result; the legacy auth import treats a database as fresh when `schema.sql` created it, which `schema.sql` records itself as the `data_fixups` row `schema-created-with-api-key-auth`, whichever process ran it) checks if the `systems` table exists in `pg_tables`. If missing (fresh database), it executes the full embedded schema. If present, it's a no-op. This runs before `db.Migrate()`. Both hold a session advisory lock (`schemaLockKey` in `schema.go`), so concurrent engines or CLI commands on one database wait for each other (INFO "another tr-engine process is setting up the database schema; waiting for it") instead of applying the same migrations twice.

**Incremental migrations** (`internal/database/migrations.go`): `db.Migrate()` runs after `InitSchema()` on every startup. Each migration has a `check` query (returns true if already applied) and idempotent `sql`. Migrations handle schema changes that post-date the initial `schema.sql` — adding columns, replacing indexes, etc. To add a new migration, append to the `migrations` slice with a `name`, `sql` (use `IF NOT EXISTS`/`IF EXISTS`), and a `check` query. On failure, `MigrationError` prints the remaining SQL for manual application by a superuser. `Migrate` applies all pending migrations; `MigrateReversible` applies all but those marked `irreversible` (`convert api_keys to app keys`, `record and drop users`: they convert a database from before API keys, which an older engine still running on it can't survive) and returns the names it left pending. The server uses `Migrate`; the CLI (`migrateForCLI` in `cmd/tr-engine/keys.go`) uses `Migrate` only with `--migrate` (`keys`/`access` refuse without it, `export`/`import` use `MigrateReversible`). `PendingMigrations` lists what `Migrate` would apply, with the `Irreversible` flag. The auth migrations read or drop a `users` table only if it is old tr-engine's (`trEngineUsersSQL`: the `trg_users_updated_at` trigger or the viewer/editor role CHECK); another application's `users` table in the same database is left alone.

**Startup order:** Connect → `InitSchema` (first-run only) → `Migrate` (every startup, skips already-applied; the CLI without `--migrate`: see above) → application boot.
...
**Subcommands:** `tr-engine keys list|create|update|revoke|import` and `tr-engine access show|set|forget-retired-token` manage API keys and the anonymous access policy directly in the database (they take `--env-file`/`--database-url` like the server; they name pending schema migrations on stderr, and on a database from before API keys refuse the irreversible conversion (`convert api_keys to app keys`, `record and drop users`) unless `--migrate` is given, since starting the new server first is the recommended order; `keys list` and `access show` print a stderr note while the one-time legacy import is still to come; `--systems`/`--talkgroups`/`--exclude-talkgroups` take comma-separated and/or repeated values that add up); `tr-engine export`/`import` move data between instances (they also name pending migrations on stderr and, on a database from before API keys, skip the irreversible ones unless `--migrate` is given: they apply the rest and leave the conversion to the server's first start, with a note on stderr; `import --dry-run` always skips them, and `--dry-run --migrate` is refused). Back up with `pg_dump` (old engine stopped), not `tr-engine export`, before upgrading: an export carries no keys, user accounts or settings. In Docker: `docker compose exec -T tr-engine tr-engine keys ...`.
...
**Auth in pages:** include `auth.js?v=3` first; plain `fetch('/api/v1/...')` and `new EventSource('/api/v1/events/stream?...')` then work with or without a stored key. Use `trAuth.mediaUrl(url)` for `<audio src>` (retry once on `error` with `await trAuth.ticketUrl(url)`), `await trAuth.ticketUrl(...)` for WebSocket URLs, and `trAuth.hasScope('edit'|'admin')` to gate features. Render API strings with `textContent` (an `edit` key can set tags to HTML). Use `Promise.allSettled` when mixing restriction-enforced and restriction-denied endpoints. A page that takes a key in its own input stores it with `trAuth.setKey(value)` or checks `trAuth.cleanKey(value).error` first, never putting the raw value in an `Authorization` header (`fetch()` throws on non-ISO-8859-1 header values).
...
**Completed:**
- `openapi.yaml` — full REST API spec with SSE event stream endpoint (75 paths, 88 operations)
- `schema.sql` — full PostgreSQL 18 DDL (27 tables, plus `config_overrides` from a migration; partitioning, triggers, helpers)
...
- REST API — every operation in `openapi.yaml` implemented; the core data endpoints live in 13 handler files (systems, talkgroups, units, calls, call_groups, stats, recorders, events/SSE, unit-events, affiliations, transcriptions, admin, query)
...
- API-key auth — The engine authenticates client software, not people. **Keys** (`tre_` + 64 hex, SHA-256 stored, plaintext shown once) carry scopes: at most one of `listen`/`edit`/`admin` (`admin` ⊃ `edit` ⊃ `listen`), optionally plus `upload` (`POST /call-upload` only). **Anonymous access policy** (`auth_settings.anonymous_access`): `off` (default) or `listen`, optionally restricted; set with `PUT /anonymous-access` or `tr-engine access set`. **Restrictions** (`allow_all`/`systems`/`talkgroups`/`exclude_talkgroups`) on `["listen"]` keys, the anonymous policy and ticket narrowings; `null` = unrestricted, empty allow list = nothing; NULL, zero and negative talkgroup or system IDs are never visible to a restricted principal (`Principal.SQL` emits `tgCol > 0 AND sysCol > 0`); at most 1000 entries per array for new input (merges can grow stored exclusion lists past it; such a stored restriction stays valid and sending it back unchanged via `PATCH /keys/{id}`, `PUT /anonymous-access` or `access set` without restriction flags is accepted); system merges rewrite them (allow entries move to the target; `exclude_talkgroups` are kept symmetric: an exclusion naming the merged-away or the surviving system names both, along chains of merges; a restriction stored later is rewritten the same way on store; a ticket narrowing naming a merged-away system anywhere, exclusions included, is refused at mint with 400 `invalid_body`). A merge can widen allow lists (they then cover the whole merged system). **Tickets** (`POST /tickets`, `trt_...`, 60 s–1 h, HMAC with `auth_settings.ticket_secret`) are accepted only as `?ticket=` on `/events/stream`, `/audio/live`, `/calls/{id}/audio` (GET/HEAD), grant `listen` only, reflect the minting key's current state, and their streams close at expiry. Presented-but-invalid credentials are always 401 (`invalid_key`/`invalid_ticket`), never anonymous. Error codes: `key_required`, `invalid_key`, `invalid_ticket` (401), `insufficient_scope`, `restricted_credential` (403), `service_unavailable` (503, key lookup failed); every 401 has `WWW-Authenticate: Bearer realm="tr-engine"`. `GET /whoami` replaces `/auth-init` and `/auth/me`; `/auth/*`, `/users` and `?token=` are gone (404). Last-admin guard on the API (not the CLI, which only warns; `keys update ID --no-rate-limit` undoes an over-tight limit): revoking, demoting, giving an earlier expiry to, or setting/lowering `rate_limit_rps` below 1 request/second on an active admin key is 409 unless another active admin key, not rate-limited below 1 rps, lasts at least as long. `keys import` refuses the retired public token (also after `forget-retired-token`), and `forget-retired-token` refuses while an active key holds it. Bootstrap admin key printed once per database. One-time legacy import (`data_fixups` `import-legacy-auth`; a database whose schema this version created, marked by `schema.sql`'s `schema-created-with-api-key-auth` row whichever process ran it, imports nothing and keeps anonymous access `off`; per-start warnings for `AUTH_TOKEN`/`WRITE_TOKEN` say what this process's value is now: "imported as API key #N" for the key the import recorded and "its value is API key #N" for another stored key (e.g. one registered with `keys import` after a rotation), each with "; that key is revoked/expired, so clients sending it get 401 invalid_key" when it no longer works; "treated as anonymous" only while the public token is still stored as retired (after `access forget-retired-token`: requests carrying it get 401 `invalid_key`); "did not import ... register it with `tr-engine keys import`" only when the value matches no stored key and no retired or forgotten token): `WRITE_TOKEN` → `legacy WRITE_TOKEN` (admin, upload) unless it equals the full-mode public `AUTH_TOKEN`; token-mode `AUTH_TOKEN` → `legacy AUTH_TOKEN` (listen, +upload without `WRITE_TOKEN`); full-mode `AUTH_TOKEN` retired (its hash is ignored when presented); anonymous policy `listen` for upgraded open instances with data and full-mode public demos. The `users` table is recorded in `data_fixups` and dropped; user-owned keys keep the lower of key and owner role.
- Audit log — `audit_log` records every key-authenticated non-GET/HEAD/OPTIONS request to a matched route that the auth layer let through (except call-upload and tickets; auth-layer refusals — 401, 403 `insufficient_scope`/`restricted_credential` — are only in the access log, handler refusals are recorded) with key name, optional `X-Actor`, method, path (no query, capped at 1024 bytes with a "…(truncated, N bytes)" marker), status and request ID; `GET /admin/audit-log`; `RETENTION_AUDIT_LOG` (also editable as `retention_audit_log` via `/admin/maintenance/config`). `unit_tag_suggestions.decided_by` and `system_merge_log.performed_by` store the key name (plus ` / <actor>`).
...
- DB maintenance — automated daily maintenance loop: partition creation (3 months ahead monthly, 3+ weeks ahead weekly; ingest creates a missing monthly partition on demand only within 12 months back to 3 ahead), state table decimation (`recorder_snapshots`, `decode_rates`: 1/min after 1 week, 1/hr after 1 month), data purging (configurable retention via `RETENTION_*` env vars), stale call cleanup, orphan call_group cleanup. Admin API: `GET /api/v1/admin/maintenance` (view config + last run results), `POST /api/v1/admin/maintenance` (trigger immediate run). Both require the `admin` scope.
...
- Auth: `Authorization: Bearer` key, `?ticket=` (browser `EventSource`), or anonymous under the anonymous policy. The stream re-checks its principal every 60 s, on every auth-generation bump and when its key reaches `expires_at` (`streamGuard`, `internal/api/streamauth.go`): it swaps the principal if `listen` remains, otherwise sends `event: auth` with `{"code": invalid_key|key_required|insufficient_scope|ticket_expired}` and closes. Ticket streams close when the ticket expires, and with `ticket_expired` when the narrowing names a since-merged-away system. A re-check that can't reach the database keeps the stream open. Clients reconnect with a fresh ticket only on `ticket_expired`. `LiveDataSource.SubscribeSince(lastEventID, filter, principal)` replaced `Subscribe`/`ReplaySince`.
- 15s keepalive comments
- Event IDs are `<unix_ms>-<32 hex>`: the bus sequence number encrypted under a per-process key (`EventBus.eventID`), so they are opaque, unique per process and not sequential; a restricted subscriber can't count the events it wasn't shown. Clients must treat them as opaque strings. (Call IDs are still one sequence across all talkgroups.)
- Ring evictions (every publish once the ring is full) and slow-subscriber drops are counted in metrics; evictions are logged at most once a minute, slow-subscriber drops once per publish for all affected subscribers.
